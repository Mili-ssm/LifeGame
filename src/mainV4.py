...
class Mundo(list):
    h: int = SIZE
    w: int = SIZE
    celulas: np.ndarray[np.ndarray[np.bool]] = field(
        default_factory=lambda: np.random.choice([0, 1], size=(2, SIZE, SIZE), p=[1.0 / 3, 2.0 / 3])
    )
            ...
            for y in range(1, self.h - 1):
                ...
                if resp:
                    pygame.draw.rect(
                        SCREEN,
                        WHITE,
                        pygame.Rect(x * BLOCKSIZE, y * BLOCKSIZE, BLOCKSIZE, BLOCKSIZE),
                    )
                else:
                    pygame.draw.rect(
                        SCREEN,
