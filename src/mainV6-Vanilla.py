...
class Mundo(list):
    __slots__ = ("celulas", "ciclo", "h", "vecinos", "w")
    ...
    def __init__(self):
        ...
        self.celulas: np.ndarray[np.ndarray[np.ubyte]] = np.random.choice(
            (0, 1), size=(SIZE, SIZE), p=[4.0 / 5, 1.0 / 5]
        )
        self.celulas[0, :] = 0
        self.celulas[:, 0] = 0
        ...
        while True:
            ...
            SCREEN.blit(
                pygame.transform.scale(
                    pygame.surfarray.make_surface(np.asarray(mundo())),
